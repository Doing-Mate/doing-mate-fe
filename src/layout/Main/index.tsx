...
import { ScheduleDetailComponent, ScheduleAddComponent } from "../../context/modalPageComponents";
...
export const Main = () => {
  ...
  const getAndSetDataList = async () => {
    ...
    const scheduleList = scheduleData.map((scheduleItem) => {
      const category = categoryData.find((categoryItem) => categoryItem.id === scheduleItem.category_id);
      return {
        ...scheduleItem,
  ...
  const eventClick = (info: EventClickArg) => {
    ...
    ScheduleDetailComponent.data = scheduleList.find((scheduleItem) => scheduleItem.id === data.publicId);
  ...
  return <Calendar events={scheduleList} eventClick={eventClick} todoPlusClick={todoPlusClick} />;
};
